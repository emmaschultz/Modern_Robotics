...
MyPclUtils::MyPclUtils(ros::NodeHandle* nodehandle) : nh_(*nodehandle), pclKinect_ptr_(new PointCloud<pcl::PointXYZ>),
...
void MyPclUtils::fit_points_to_plane(Eigen::MatrixXf points_mat, Eigen::Vector3f &plane_normal, double &plane_dist) {
...
void MyPclUtils::fit_points_to_plane(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr, Eigen::Vector3f &plane_normal, double &plane_dist) {
...
Eigen::Vector3f MyPclUtils::compute_centroid(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr) {
    Eigen::Vector3f centroid;
    Eigen::Vector3f cloud_pt;
    int npts = input_cloud_ptr->points.size();
    centroid<<0,0,0;
...
void MyPclUtils::fit_xformed_selected_pts_to_plane(Eigen::Vector3f &plane_normal, double &plane_dist) {
...
Eigen::Affine3f MyPclUtils::transformTFToEigen(const tf::Transform &t) {
    Eigen::Affine3f e;
 ...
 */
void MyPclUtils::transform_kinect_cloud(Eigen::Affine3f A) {
...
void MyPclUtils::transform_selected_points_cloud(Eigen::Affine3f A) {
...
void MyPclUtils::get_transformed_selected_points(pcl::PointCloud<pcl::PointXYZ> & outputCloud ) {
...
void MyPclUtils::example_pcl_operation() {
...
void MyPclUtils::copy_cloud(PointCloud<pcl::PointXYZ>::Ptr inputCloud, PointCloud<pcl::PointXYZ>::Ptr outputCloud) {
...
void MyPclUtils::transform_cloud(Eigen::Affine3f A, pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr,
...
void MyPclUtils::initializeSubscribers() {
    ROS_INFO("Initializing Subscribers");

    pointcloud_subscriber_ = nh_.subscribe("/kinect/depth/points", 1, &MyPclUtils::kinectCB, this);
    // add more subscribers here, as needed

    // subscribe to "selected_points", which is published by Rviz tool
    selected_points_subscriber_ = nh_.subscribe<sensor_msgs::PointCloud2> ("/selected_points", 1, &MyPclUtils::selectCB, this);
}

//member helper function to set up publishers;

void MyPclUtils::initializePublishers() {
 ...
 */
void MyPclUtils::kinectCB(const sensor_msgs::PointCloud2ConstPtr& cloud) {
...
void MyPclUtils::selectCB(const sensor_msgs::PointCloud2ConstPtr& cloud) {
